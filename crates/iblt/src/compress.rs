...
pub fn compress(data: &[u8], config: &CompressConfig) -> Vec<u8> {
    if data.len() < config.min_size_threshold {
        return data.to_vec();
    }
    match config.algorithm {
        CompressionAlgorithm::None => data.to_vec(),
        CompressionAlgorithm::Rle => rle_compress(data),
        CompressionAlgorithm::Delta => delta_compress(data),
    }
}
pub fn decompress(data: &[u8], alg: CompressionAlgorithm) -> anyhow::Result<Vec<u8>> {
    match alg {
        CompressionAlgorithm::None => Ok(data.to_vec()),
        CompressionAlgorithm::Rle => rle_decompress(data),
        CompressionAlgorithm::Delta => delta_decompress(data),
    }
}
...
fn delta_compress(data: &[u8]) -> Vec<u8> {
    ...
    r.push(data[0]);
    for i in 1..data.len() {
        r.push(data[i].wrapping_sub(data[i - 1]));
    }
    r
}
fn delta_decompress(data: &[u8]) -> anyhow::Result<Vec<u8>> {
    if data.is_empty() {
        return Ok(Vec::new());
    }
    let mut r = Vec::with_capacity(data.len());
    r.push(data[0]);
    for i in 1..data.len() {
        r.push(r[i - 1].wrapping_add(data[i]));
    }
    Ok(r)
}
...
mod tests {
    ...
    fn test_rle_roundtrip() {
        let data = vec![0, 0, 0, 0, 1, 1, 2, 2, 2, 2, 2];
    ...
    fn test_delta_roundtrip() {
        ...
        let c = delta_compress(&data);
        assert_eq!(delta_decompress(&c).unwrap(), data);
    }
}
