...
pub fn reassemble_chunks(chunks: &[IbltChunk]) -> anyhow::Result<Vec<u8>> {
    ...
    if chunks.len() != total {
        anyhow::bail!("expected {} chunks, got {}", total, chunks.len());
    }
...
mod tests {
    ...
    fn test_prepare_and_reassemble() {
        let sender = IbltSender::new(SendConfig {
            chunk_size: 32,
            compress: false,
            ..Default::default()
        });
