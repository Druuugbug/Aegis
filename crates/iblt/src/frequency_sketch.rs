...
impl FrequencySketch {
    pub fn new(num_rows: usize, num_cols: usize) -> Self {
        let seeds: Vec<u64> = (0..num_rows)
            .map(|i| 0x9e3779b97f4a7c15u64.wrapping_mul(i as u64 + 1))
            .collect();
        Self {
            table: vec![vec![0u64; num_cols]; num_rows],
            num_rows,
            num_cols,
            seeds,
            total_count: 0,
    ...
    pub fn add(&mut self, key: &[u8]) {
        for row in 0..self.num_rows {
            let idx = self.hash_index(key, row);
...
mod tests {
    ...
    #[test]
    fn test_frequency() {
        let mut s = FrequencySketch::new(4, 256);
        s.add(b"hot");
        s.add(b"hot");
        s.add(b"hot");
        assert!(s.estimate(b"hot") >= 3);
    }
    #[test]
    fn test_clear() {
        let mut s = FrequencySketch::new(4, 128);
        s.add(b"test");
        s.clear();
        assert_eq!(s.estimate(b"test"), 0);
    }
}
