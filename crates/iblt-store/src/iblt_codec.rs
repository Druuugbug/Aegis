...
#[repr(u8)]
pub enum StorageTierCode {
    Hot = 1,
    Warm = 2,
    Cold = 3,
}
impl StorageTierCode {
...
pub struct StoreCodecConfig {
    pub schema_version: u16,
    pub include_checksum: bool,
}
impl Default for StoreCodecConfig {
    fn default() -> Self {
        Self {
            schema_version: 1,
...
pub struct StoreCodec {
    config: StoreCodecConfig,
}
impl StoreCodec {
    pub fn new(config: StoreCodecConfig) -> Self {
        Self { config }
    }
    ...
    pub fn decode(&self, data: &[u8]) -> anyhow::Result<(CompactIblt, StorageTierCode, u16)> {
        ...
        if data[0] != 0x53 || data[1] != 0x49 {
            anyhow::bail!("bad marker");
        }
        ...
        if self.config.include_checksum {
            ...
            if stored != simple_checksum(payload) {
...
impl Default for StoreCodec {
    fn default() -> Self {
        Self::new(StoreCodecConfig::default())
    }
}
...
mod tests {
    use super::*;
    #[test]
    fn test_store_codec_roundtrip() {
        let codec = StoreCodec::default();
        let mut iblt = CompactIblt::new(64);
        iblt.insert(b"t", b"v");
        let enc = codec.encode(&iblt, StorageTierCode::Hot);
        let (dec, tier, sv) = codec.decode(&enc).unwrap();
        assert_eq!(tier, StorageTierCode::Hot);
        assert_eq!(sv, 1);
    }
    #[test]
    fn test_tier_codes() {
        assert_eq!(StorageTierCode::from_u8(1), Some(StorageTierCode::Hot));
        assert_eq!(StorageTierCode::from_u8(0), None);
    }
}
