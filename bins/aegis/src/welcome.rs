...
pub fn print_welcome_banner(version: &str, model: &str, session_id: &str) {
    ...
    let stripe_char = |row: usize, col: usize| -> char {
        if let Some(c) = noise_char(row, col) {
            return c;
        }
    ...
    let art_max_chars = art_lines
        .iter()
        .map(|l| l.chars().count())
        .max()
        .unwrap_or(77);
    let art_start = if term_width > art_max_chars {
        (term_width - art_max_chars) / 2
    } else {
        0
    };
    ...
    let info_row1_plain = format!(
        "  Aegis v{} \u{00b7} {} \u{00b7} session {}  ",
        version, model, session_id
    );
...

    {
        ...
        for col in 0..term_width {
            out.push(stripe_char(base_row + 2, col));
        }
        out.push_str(RESET);
        eprintln!("{}", out);
    }
